...
def respond(
    ...
    if msg_type == "PING":
      return_msg = {"msg_type": "PONG", "ttl": 0, "data": None}
      send_message_to(msg_forwarder, return_msg, False)
    elif msg_type == "PONG":
      pass
    elif msg_type == "PRIME":
      update_last_heard_from(msg_originator)
      
      originator_prime = data
      if originator_prime > STATE["biggest_prime"]:
        STATE["biggest_prime"] = originator_prime
      ...
      if ttl > 0:
        for peer in STATE["peers"]:
          forwarded_msg = {
            ...
            "ttl": ttl - 1,
            "data": data}
          send_message_to(peer, forwarded_msg, True)
    else:
      raise ValueError("Unrecognized message type '%s'" % msg_type)
